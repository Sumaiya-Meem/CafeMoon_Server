...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.ojnnavp.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    const reviewCollection = client.db('CafeMoonDB').collection('reviews');
    const userCollection = client.db('CafeMoonDB').collection('users');
